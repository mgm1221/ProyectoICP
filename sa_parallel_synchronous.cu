...
__global__ void simulated_annealing(char* flat_sequences, int MAX_K, int WIDTH, int HEIGHT, int n, int m, float th, char* results, float* qualities,
                                    ...
                                    char* current_s, int num_threads,int best_index, float T, int iterations) {
    ...
    //en caso del paso inicial, se selecciona una solucion aleatoria
    if(best_index < 0){
        //generamos la solucion aleatoria
        random_sequence(&current_s[tid * m], m);
        //se copia la solucion actual a la mejor solucion
        for (int i = 0; i < m; ++i) { 
            best_s[(tid * m) + i] = current_s[(tid * m) + i];
        }
    }else{
        //si ya hay una mejor solucion, se copia
        for (int i = 0; i < m; ++i) { 
            best_s[(tid * m) + i] = best_s[(best_index* m)+ i];
            current_s[(tid * m) + i] = best_s[(best_index * m) + i];
        }
    }
    float current_cost = h(&best_s[tid * m], flat_sequences, MAX_K, WIDTH, HEIGHT, n, m, th, d_array, c_array, tid);
...
int main(int argc, char *argv[])
{
    ...
    string iteration_args = argv[13];
    int iterations = stoi(argv[14]);
    ...
    float min_T = 1e-3; // Temperatura mínima
    int best_index = -1; // Indice de la mejor solucion, -1 indica que no hay una mejor solucion aun
    ...
    while(T > min_T) {
        ...
        simulated_annealing<<<blocks_per_grid, threads_per_block>>>(device_flat_sequences,
                                                                    ...
                                                                    device_d_array, device_c_array, 
                                                                    d_neighbors_flat, d_best_s_flat, d_current_s_flat,
                                                                     num_threads,best_index,T,iterations);
